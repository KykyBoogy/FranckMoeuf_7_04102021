...
// Initialisation des arrays appareils, ustensils, ingredients.
const setArray = (recipes) => {

    recipes.forEach(recipe => {
        //Initialisation de la liste des appareils, 
        // uniquement ceux qui ne sont pas inclus dans arrayOfAppliance.
        if(arrayOfAppliance.indexOf(recipe.appliance) === -1) {

            arrayOfAppliance.push(recipe.appliance);
           
        }
        //Initialisation de la liste des ustanciles, 
        // uniquement ceux qui ne sont pas inclus dans arrayOfUstensils.
        recipe.ustensils.map((ustensil) => {

            if(!arrayOfUstensils.includes(ustensil)) {

                arrayOfUstensils.push(ustensil);
            
            }
        });    
        //Initialisation de la liste des ingrédients, 
        // uniquement ceux qui ne sont pas inclus dans arrayOfIngredients.
...
const setTags = () => {
    ...
    arrayOfIngredients.forEach((ingredient, index) => {
        ...
        $('#dropdownIngredientList')
            .append(`<li><a href="#" id="ingredient-${index}">${ingredient}</a></li>`);

    });
    // Liste des appareils.
    arrayOfAppliance.forEach((appliance, index) => {

        $('#dropdownAppareilList')
            .append(`<li><a href="#" id="appliance-${index}">${appliance}</a></li>`);

    });
    // Liste des ustanciles.
    arrayOfUstensils.forEach((ustensil, index) => {

        $('#dropdownUstensilList')
            .append(`<li><a href="#" id="ustensil-${index}">${ustensil}</a></li>`);
...
const setRecipesHTML = (recipes) => {
    ...
    recipes.forEach((recipe, index) => {
                ...
                <div class="card mb-4 box-shadow no-border">
                    ...
                    <div class="card-body card-body-style">
                        <div class="d-flex justify-content-between">
                            ...
                            <div class="card-time"><img src="./assets/img/clock-icon.png" class="icon clockIcon" /> <span>${recipe.time} min</span></div>
                        </div>
        ...
        `);
        // Initialisation de la liste des ingrédients, 
        // uniquement les ingrédients qui ne sont pas présents dans arrayOfIngredients.
        // On ajoute les ingrédients associés a l'ID de la recette.
        recipe.ingredients.forEach(i => 
            $(`#id-card-ingredients-${recipe.id}`)
                .append(`<li>${i.ingredient}: 
                <span>${i.quantity ? i.quantity : ''} ${i.unit ? i.unit : ''}
                </span></li>`));
...
// Utilisation des constantes recipesJson
setArrayOfRecipes(recipesJson);
setArray(recipesJson);
setRecipesHTML(recipesJson);

// Afficher/Masquer les list dropdown.
const toggleDropdownList = (dropdownList, buttonId) => {
    
    isVisible = !isVisible;
    
    const dropdownListElement = document.querySelector(`#${dropdownList}`);
    ...
    if(dropdownListElement.style.display === 'block') {
...
// Switch icones des liste dropdown.
const toggleDropdownIcon = (showList) => {

    let booleanShowList = showList;
    // Icone vers le haut si on trouve un résultat.
    if(booleanShowList) {

        $(".icon-dropdown-down").css("display", "none");
        $(".icon-dropdown-up").css("display", "inherit"); 
    // Icone vers le bas par défaut.
    } else {
...
// Afficher/Masquer les tags dans les dropdown list selon la saisie.
const filterByDropdownText = (inputElem, dropdownList) => {
    ...
    // Si la valeur saisie est égale ou supérieur a 3
    // On modifie le style de la list ul
    // et on affiche les tags trouvés.
    if(input.value.length >= 3) {

        $(`#${dropdownList}`).css("display", "flex");
        ul = document.getElementById(dropdownList);
        li = ul.getElementsByTagName("li");
        // On parcours tous les tags contenu dans les balises <li>.
        for (i = 0; i < li.length; i++) {

            a = li[i].getElementsByTagName("a")[0];
            txtValue = a.textContent || a.innerText;
            // Si la valeur saisie est supérieur a l'index -1
            // Alors on affiche le tag,
            // sinon on le masque.
            ...
            // Si non a un résultat l'icone flèche sera vers le haut.
            if(i === li.length -1) {
...
$(".dropDownList").on("click", "li", function(event) {
    ...
    let tagList = $("#tags");
    let colorBtn;
    let tagTarget = event.delegateTarget.id;

    if(!arrayTagId.includes(event.target.id)) {
        // On push les id, pour gérer la suppression.
        arrayTagId.push(event.target.id); 
        arrayTagValue.push(event.target.text);

        // Modification du background en fonction des catégories.
        if(tagTarget == 'dropdownIngredientList') colorBtn = 'btnBlue';
        if(tagTarget == 'dropdownAppareilList') colorBtn = 'btnGreen';
        if(tagTarget == 'dropdownUstensilList') colorBtn = 'btnOrange';
    ...
    filterRecipesByTags(arrayTagValue);
    
});

                // ========== Algorithme de recherche Version 1. =========== //

// Recherche des recettes dans le titre de la recette, 
// la liste des ingrédients, des appareils et ustensiles d'une recette
// ainsi que sa description.
const filterAllByText = () => {
    // On récupère la valeur saisie dans l'input de recherche.
    let searchValue = document.getElementById("inputSearchAll").value.toLowerCase();
    // A partir de 3 caractères saisies.
    if(searchValue.length >= 3) {
        // On récupère toutes les recettes
        // ou recettes filtrées par tags.
        arrayOfRecipesFilteredByText = arrayOfRecipesFilteredByTag 
                                        && arrayOfRecipesFilteredByTag.size > 0 ? 
                                        [...arrayOfRecipesFilteredByTag] : arrayOfRecipes;
        
        // Recherche recettes par appareil, par nom et dans la description.
        recipesByAppliance = arrayOfRecipesFilteredByText.filter(currentElement => {

            return currentElement.appliance.toLowerCase().includes(searchValue) 
                || currentElement.name.toLowerCase().includes(searchValue) 
                || currentElement.description.toLowerCase().indexOf(searchValue) >= 0;

        });
        
        // Recherche recettes par ingrédients.
        recipesByIngredients = arrayOfRecipesFilteredByText
            .filter(recipe => recipe.ingredients
                .filter(i => i.ingredient.toLowerCase()
                .includes(searchValue)).length > 0);
                
                
        // Recherche recettes par ustensils.
        recipesByUstensils = arrayOfRecipesFilteredByText.filter(recipe => {

            return recipe.ustensils.some(ustensil => ustensil.toLowerCase().includes(searchValue));
    ...
    } else if (searchValue.length <= 2 && arrayOfRecipesFilteredByTag 
        && arrayOfRecipesFilteredByTag.size > 0) {
...
const filterRecipesByTags = (tags) => {
    ...
    if(tags.length > 0) {
        // On regarde si le tags existe dans le texte et on le stock.
        arrayOfRecipesFilteredByTag = arrayOfRecipesFilteredByText 
            && arrayOfRecipesFilteredByText.size > 0 ? 
            [...arrayOfRecipesFilteredByText] : arrayOfRecipes;
        
        // Recettes filtrées par appareils.
        ...
        // Recettes filtrées par ingredients.
        recipesByIngredients = arrayOfRecipes.filter(recipe => 
            recipe.ingredients.filter(i => tags.indexOf(i.ingredient) >= 0).length > 0);

        // Recettes filtrées par ustensils.
        recipesByUstensils = arrayOfRecipes.filter(recipe => {

            return recipe.ustensils.some(ustensil => tags.indexOf(ustensil) >= 0);

        });

        // On stock les tags pour chaque catégories.
        arrayOfRecipesFilteredByTag = new Set([].concat(recipesByAppliance, recipesByIngredients, recipesByUstensils));

        // Si on ne trouve pas de tags (array vide).
        // Alors on affiche "recettes non trouvées".
...
// Afficher/Masquer les recettes filtrées.
...
const deleteTagById = (elementId) => {
    // Pour chaque tag qui a été cliqué
    // on retire l'élément html.
    for( let i = 0; i < arrayTagId.length; i++) {

        if ( arrayTagId[i] === elementId) {

            $(`.${elementId}`).remove();
            arrayTagValue.splice(i, 1);
            arrayTagId.splice(i, 1);

            // Si on supprime un tag,
            // on le retire du tableau,
            // et on met à jour les filtres.
            if(arrayTagValue && arrayTagValue.length == 0) {

                arrayOfRecipesFilteredByTag = [];
                filterRecipesByTags(arrayTagValue);
